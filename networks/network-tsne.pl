...
my $nfile = ''; # coordinates file from t-SNE
...
if($#ARGV==0) {
  ...
  print "-g [rank associated with each node]\n";
	print "-n [coordinates from t-SNE step]\n";
...
#get node categories forr each gene
...
open my $coordinatesFH, '<', $nfile or die "Could not open $nfile: $!";
my $coordinatesTSV = Text::CSV_XS->new({
	sep_char => "\t",
});
$coordinatesTSV->getline($coordinatesFH); # discard header
my @nodeArray;
my %nodes;
while(my $row = $coordinatesTSV->getline($coordinatesFH)) {
	my %nodeHash = (
    'name' => @{$row}[0],
    'x' => @{$row}[1],
    'y' => @{$row}[2],
  );
	$nodes{@{$row}[0]}{'x'} = $nodeHash{x};
	$nodes{@{$row}[0]}{'y'} = $nodeHash{y};
...
open my $networkFH, '>', 'nmf-tsne-network.json';
print $networkFH "{\n\t\"nodes\": [\n";
...
open my $networkLegendFH, '>', 'nmf-tsne-network-legend.json';
